...
/**
 * Created by gensh on 2016/3/25.
 */

public class SelectWheel extends View implements GestureDetector.OnGestureListener {
    ...
    public SelectWheel(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void init(AttributeSet attrs) {
        ...
        if (ta != null) {
            entries = ta.getTextArray(R.styleable.lwvWheelView_android_entries);
    ...
    private void calcIntervalDis() {
        ...
        if (entries != null && entries.length > 0) {
            for (CharSequence i : entries) {
                String s = i.toString();
                mMarkTextPaint.getTextBounds(s, 0, s.length(), temp);
                if (temp.width() > max) {
                    max = temp.width();
                }
            }
    ...
    public void fling(int velocityX, int velocityY) {
        mScroller.fling(getScrollX(), getScrollY(),
                velocityX, velocityY,
                (int) (-mMaxOverScrollDistance + mMinSelectableIndex * mIntervalDis),
                (int) (mContentRectF.width() - mMaxOverScrollDistance - (mMarkCount - 1 - mMaxSelectableIndex) * mIntervalDis),
                0, 0, (int) mMaxOverScrollDistance, 0);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (entries == null || entries.length == 0 || !isEnabled()) {
            return false;
        }
    ...
    public void setItems(CharSequence[] items) {
        ...
        entries = items;
        mMarkCount = null == entries ? 0 : entries.length;
        if (mMarkCount > 0) {
            mMinSelectableIndex = Math.max(mMinSelectableIndex, 0);
            mMaxSelectableIndex = Math.min(mMaxSelectableIndex, mMarkCount - 1);
        }
    ...
    public interface OnWheelItemSelectedListener {
        void onWheelItemChanged(SelectWheel wheelView, int position);

        void onWheelItemSelected(SelectWheel wheelView, int position);
    }
